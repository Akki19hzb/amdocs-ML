...
from config import Sachin_config,Kohli_config,config
...
def break_total_runs(config1, config2):
    """

    :param config1: The player that has lower runs like kholi
    :param config2: The player with more runs like sachin
    """
    while(config1.total_runs<config2.total_runs):
        model_1 = load_from_pickle(config1.name_of_model)
        simulate_n_matches(model_1,config1,1)
    print_current_stats(config1)
...
if player_selection=='1':
    ...
    model=load_from_pickle(Kohli_config.name_of_model)
else:
...
while selection!='4':
    selection=input('Select the type of analysis.\n1.Simulate N matches'
                ...
                '\n3.Break total runs for Kohli only'
                '\n4.Exit')
    if selection=='1':
        ...
        if player_selection=='2':
          simulate_n_matches(model,Kohli_config,int(n_matches))
    elif selection=='2':
        if player_selection=='1':
         print_current_stats(Sachin_config)
        if player_selection=='2':
          print_current_stats(Kohli_config)
    elif selection == '3':
            break_total_runs(Kohli_config,Sachin_config)
    elif selection=='4':
        print('Exit')
        exit()
